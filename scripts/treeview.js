...
(function ($) {
    ...
    function _exportData($node, deep, onlyChecked) {
        ...
        if ($this[0].tagName.toLowerCase() == 'li') {
            ...
            deep = (($this.data('status') == 'mixed')) || onlyChecked;
                ...
                if (hasStatus == 1)
                    ...
                    id: $this.data('id')
                     ...
                     , c: $this.data('status') == 'mixed'
                         , d: $this.find('> .node_content').text()
                     , ch: null
                    }
                else
                    return (deep && ($this.find('> ul').length > 0)) ? _exportData($this.find('> ul'), deep, onlyChecked) : null;
            } else {
            ...
            $this.find('> li').each(function () {
                    ...
                    if (node != null)
                        if (onlyChecked  && node.id!=undefined ) {
                            nodes = node;
                            return false; //don't look for more after found node.
                        }
                        else
                        nodes.push(node);
            });
